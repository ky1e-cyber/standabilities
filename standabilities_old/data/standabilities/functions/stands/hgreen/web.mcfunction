...
execute as @e[tag = sa_hgnode, tag = !sa_hgspawn] run scoreboard players set @s sa_distance 0
execute as @e[tag = sa_hgnode, tag = !sa_hgspawn] positioned as @s facing entity @e[tag = sa_hgspawn, limit = 1] eyes positioned ^ ^ ^0.5 run function standabilities:stands/hgreen/web_ray
